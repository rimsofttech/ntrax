                                ...
                                <div class="page-title-box">
                                    <div class="page-title-right">
                                        <ol class="breadcrumb m-0">
                                            ...
                                            <li class="breadcrumb-item"><a href="javascript: void(0);">SubProduct</a></li>
                                            {{-- <li class="breadcrumb-item active">Datatables</li> --}}
                                        </ol>
                                    </div>
                                    <h4 class="page-title">SubProduct Master</h4>
                                </div>
                            </div>
                        </div>     
                        <!-- end page title --> 
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-sm-4">
                                                ...
                                                $createsubproduct = ''; 
                                                $createtext = '';
                                                if(!Entrust::can('Create SubProduct'))
                                                {
                                                    $createsubproduct = ''; 
                        ...
                        <div class="modal-dialog modal-dialog-centered" role="document">
                          <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <form method="post" id="subproduct_form" class="form-horizontal" enctype="multipart/form-data">
                                        @csrf
                                    <div class="alert alert-success d-none" id="msg_div">
                                          <span id="res_message"></span>
                                     </div>
                                     <div class="form-group">
                                        <label for="channeltype">Select Product Name</label>
                                        <select class="form-control"  data-placeholder="Choose Product Name..." id="product_name" name="product_name" style="width:100%">
...
@section('script')
       ...
       <script>
        $(document).ready(function(){
         ...
         var addsubproductcheck = "{{ Entrust::can('Create SubProduct') ? 'true' : 'false' }}";
        ...
        var erroraddcheck = '';
        if(addsubproductcheck!='true')
        {
            erroraddcheck = 'You do not have proper permission to Create Record';
        }
        datatable.button().add(null, {className:'btn btn-danger',text:'<i class="mdi mdi-plus-circle mr-1"></i> Add SubProduct',className:'btn btn-success floatRight align-right',attr:{title:erroraddcheck,id:'create_record'}});
         $('#create_record').click(function(){
            var createsubproduct = "{{Entrust::can('Create SubProduct')? true : false }}";
                if(createsubproduct != true)
                {
                    unauthorized();
                    return false;
                }
        ...
        $('#subproduct_form').on('submit', function(event){
            event.preventDefault();
            if($('#action').val() == 'Add')
            {
            $.ajax({
                url:"{{ route('subproduct.store') }}",
                method:"POST",
        ...
        $(document).on('click', '.edit', function(){
            var editsubproduct = "{{Entrust::can('Edit SubProduct')? true : false }}";
            ...
            $("#product_name option:selected").attr('selected',false).trigger('change');
            $('#name').val('');
            
            var id = $(this).attr('id');
            $('#form_result').html('');
            $.ajax({
            url:"/admin/subproduct/"+id+"/edit",
            dataType:"json",
            success:function(html){
                //console.log(html);
                $('#product_name').append("<option value="+html.data.product_id+" selected>"+html.data.productname+"</option>").trigger('change');
            ...
            $(document).on('click', '.delete', function(){
                var deletesubproduct = "{{Entrust::can('Delete SubProduct')? true : false }}";
            ...
            $.ajax({
            url:"/admin/subproduct/destroy/"+subproduct_id,
            beforeSend:function(){
