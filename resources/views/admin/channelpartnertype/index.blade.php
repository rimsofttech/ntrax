...
@section('css')
        ...
        <title>Ntrax - Channel Partner Type</title>
                                ...
                                <div class="page-title-box">
                                    <div class="page-title-right">
                                        <ol class="breadcrumb m-0">
                                            ...
                                            <li class="breadcrumb-item"><a href="javascript: void(0);">Channel Partner Type</a></li>
                                            {{-- <li class="breadcrumb-item active">Datatables</li> --}}
                                        </ol>
                                    </div>
                                    <h4 class="page-title">Channel Partner Type Master</h4>
                                </div>
                            </div>
                        </div>     
                        <!-- end page title --> 
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-sm-4">
                                                @php  
                                                $createchannelpartnertype = ''; 
                                                $createtext = '';
                                                if(!Entrust::can('Create ChannelPartnerType'))
                                                {
                                                    $createchannelpartnertype = ''; 
                                        ...
                                        <table  class="table table-striped nowrap fetchdata" width="100%">
                                            <thead>
                                                <tr>
                                                    <th>Sr No.</th>
                                                    <th>Channel Partner Type Name</th>
                                                    <th>Created At</th>
                                                    <th>Updated At</th>
                                                    <th>Action</th>
                        ...
                        <div class="modal-dialog modal-dialog-centered" role="document">
                          <div class="modal-content">
                            <div class="modal-header">
                              <h5 class="modal-title" id="ModalLongTitle">Add New Channel Partner  Type</h5>
                            ...
                            <div class="modal-body">
                                ...
                                <form method="post" id="channelpartnertype_form" class="form-horizontal" enctype="multipart/form-data">
                                        @csrf
                                  ...
                                  <div class="form-group">
                                    ...
                                    <input type="text" name="name" class="form-control" id="name" placeholder="Please enter Channel Partner Type name">
...
@section('script')
       ...
       <script>
        $(document).ready(function(){
         ...
         var addchannelpartnertypecheck = "{{ Entrust::can('Create ChannelPartnerType') ? 'true' : 'false' }}";
         datatable =$('.fetchdata').DataTable({
          ...
          scrollX: true,
          ajax:{
           url: "{{ route('channelpartnertype.index') }}",
          },
        //   data: data,
          columns:[
            {
        ...
        var erroraddcheck = '';
        if(addchannelpartnertypecheck!='true')
        {
            erroraddcheck = 'You do not have proper permission to Create Record';
        }
        datatable.button().add(null, {className:'btn btn-danger',text:'<i class="mdi mdi-plus-circle mr-1"></i> Add ChannelPartnerType',className:'btn btn-success floatRight align-right',attr:{title:erroraddcheck,id:'create_record'}});
         $('#create_record').click(function(){
            var createchannelpartnertype = "{{Entrust::can('Create ChannelPartnerType')? true : false }}";
                if(createchannelpartnertype != true)
                {
                    unauthorized();
                    return false;
                }
        ...
        $('#channelpartnertype_form').on('submit', function(event){
            event.preventDefault();
            if($('#action').val() == 'Add')
            {
            $.ajax({
                url:"{{ route('channelpartnertype.store') }}",
                method:"POST",
                ...
                $('#channelpartnertype_form')[0].reset();
        ...
        $.ajax({
            url:"{{ route('channelpartnertype.update') }}",
            method:"POST",
            data:new FormData(this),
            contentType: false,
            cache: false,
        ...
        $(document).on('click', '.edit', function(){
            var editezone = "{{Entrust::can('Edit ChannelPartnerType')? true : false }}";
                if(editezone != true)
                {
                    unauthorized();
                    return false;
            ...
            $('#form_result').html('');
            $.ajax({
            url:"/admin/channelpartnertype/"+id+"/edit",
            dataType:"json",
            ...
            $(document).on('click', '.delete', function(){
                var deletechannelpartnertype = "{{Entrust::can('Delete ChannelPartnerType')? true : false }}";
                if(deletechannelpartnertype != true)
                {
                    unauthorized();
                    return false;
                }
            channelpartnertype_id = $(this).attr('id');
            $('#ok_button').text('Ok');
            ...
            $.ajax({
            url:"/admin/channelpartnertype/destroy/"+channelpartnertype_id,
            beforeSend:function(){
